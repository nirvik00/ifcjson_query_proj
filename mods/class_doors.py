...
class door_obj(object):
    def __init__(self, data):
        self.data = data
        self.num = 0
        self.simplified_data_arr = []  # array of all simple space objects
        #
        self.get_doors()  # run the functions
        #
        self.out = {"doors": self.simplified_data_arr}
        #

    def get_doors(self):
        a = parse("$..data[?(@.type=='Door')]").find(self.data)
        for i in a:
    ...
    def repr_json(self, json2):
        ...
        try:
            ...
            pass
        try:
            out['is_fire_exit'] = parse(
                "$.IsFireExit").find(json2)[0].value
        except:
            pass
        try:
        ...
        try:
            ...
            pass
        try:
            out['trim_projection_ext'] = parse(
                "$.'Trim Projection Ext'").find(json2)[0].value
        except:
            pass
        try:
            out['trim_projection_int'] = parse(
                "$.'Trim Projection Int'").find(json2)[0].value
        except:
            pass
        try:
            out['trim_width'] = parse(
                "$.'Trim Width'").find(json2)[0].value
        except:
            pass
        try:
