...
class roof_obj(object):
    def __init__(self, data):
        ...
        #
        self.out = {"roofs": self.simplified_data_arr}
        #

    def get_roofs(self):
        a = parse("$..data[?(@.type=='Roof')]").find(self.data)
        for i in a:
    ...
    def repr_json(self, json2):
        ...
        try:
            ...
            pass
        try:
            out['total_area'] = parse("$.TotalArea").find(json2)[0].value
        except:
            pass
        try:
            out['projected_area'] = parse(
                "$.ProjectedArea").find(json2)[0].value
        except:
            pass
        try:
