...
class wall_obj(object):
    def __init__(self, data, get_obj=False):
        ...
        #
        self.get_walls()  # run the functions
        #
        self.out = {"walls": self.simplified_data_arr}
        #

    def get_walls(self):
        a = parse("$..data[?(@.type=='WallStandardCase')]").find(self.data)
        for i in a:
    ...
    def repr_json(self, json2):
        ...
        try:
            ...
            pass
        try:
            out['unconnected_height'] = parse(
                "$.'Unconnected Height'").find(json2)[0].value
        except:
            pass
        try:
        ...
        try:
            ...
            pass
        try:
            out['base_level_constraint'] = parse(
                "$.'Base Constraint'").find(json2)[0].value
        except:
            pass
        try:
            out['top_level_constraint'] = parse(
                "$.'Top Constraint'").find(json2)[0].value
        except:
            pass
        #
        # openings
